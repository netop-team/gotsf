...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        # load parameters
        c_in = configs.enc_in
        context_window = configs.seq_len + 2 if self.use_deltas else configs.seq_len
        target_window = 2 * configs.pred_len if self.use_interval_classification else configs.pred_len
    ...
    def forward(self, x, deltas_l=None, deltas_u=None):  # x: [Batch, Input length, Channel]
        if self.decomposition:
            ...
            if self.use_deltas and deltas_l is not None and deltas_u is not None:
                ...
                deltas_l = torch.tensor(deltas_l.repeat(1, x.shape[1], 1), device=x.device, dtype=x.dtype)
                deltas_u = torch.tensor(deltas_u.repeat(1, x.shape[1], 1), device=x.device, dtype=x.dtype)
                x = torch.cat((x, deltas_l, deltas_u), dim=-1)

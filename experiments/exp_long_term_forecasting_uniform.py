...
class Exp_Long_Term_Forecast_Uniform(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_Uniform, self).__init__(args)
    ...
    def _select_criterion(self):
        dist_min_max = self.args.w_hat * (self.args.train_interval_u - self.args.train_interval_l)
        rand_val1 = np.random.rand(self.args.batch_size)
        rand_val2 = np.random.rand(self.args.batch_size)
        deltas1 = (self.args.train_interval_u - dist_min_max - self.args.train_interval_l) * rand_val1 + self.args.train_interval_l
        deltas2 = (self.args.train_interval_u - deltas1 - dist_min_max) * rand_val2 + deltas1 + dist_min_max
        rand_intervals = np.stack([deltas1, deltas2], axis=1)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in (
                ...
                criterion, deltas_l, deltas_u = self._select_criterion()
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        outputs_reg = outputs[:, -self.args.pred_len:, f_dim:]
                        loss = criterion(outputs_reg, batch_y)

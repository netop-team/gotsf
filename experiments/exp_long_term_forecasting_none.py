...
from utils.tools import EarlyStopping, adjust_learning_rate, visual, compute_global_min_max
...
class Exp_Long_Term_Forecast_None(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_None, self).__init__(args)
    ...
    def _select_criterion(self):
        criterion = nn.L1Loss() if self.args.loss == 'MAE' else nn.MSELoss()
        return criterion
    ...
    def vali(self, vali_data, vali_loader, criterion, flag):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in (
                    tqdm(enumerate(vali_loader), desc=f'Validation on {flag}: ', total=len(vali_loader), ncols=100)):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            if not self.args.enable_fast_train:
                vali_loss = self.vali(vali_data, vali_loader, criterion, 'vali')
